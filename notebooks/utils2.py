    ...
    for b_idx in range(y_train.shape[2]):
        ...
        if BEHAVIOR_NAMES[b_idx] == 'Grooming':
            sav = ModelCheckpoint(filepath='resnet_lstm_accuracy_grooming.h5', verbose=1,
                                  save_best_only=True, save_freq='epoch', monitor='loss', )
        elif BEHAVIOR_NAMES[b_idx] == 'Rearing':
            sav = ModelCheckpoint(filepath='resnet_lstm_accuracy_rearing.h5', verbose=1,
                                  save_best_only=True, save_freq='epoch', monitor='loss', )
        else:
